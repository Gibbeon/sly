...
D3D12VertexBufferImpl::D3D12VertexBufferImpl(ref_t<D3D12DeviceImpl> device) :
    D3D12ManagedImpl(device) 
{
    //D3D12ResourceImpl::init(desc);
}

void D3D12VertexBufferImpl::init(ref_t<VertexBufferDesc> desc) {
...
void D3D12VertexBufferImpl::write(ptr_t data, size_t size, size_t stride) {
    ubyte* dest;
